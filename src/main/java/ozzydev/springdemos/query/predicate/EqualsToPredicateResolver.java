...
public class EqualsToPredicateResolver implements PredicateResolver
{
    @Override
    public <T> Predicate resolve(Root<?> root, CriteriaQuery<?> query, CriteriaBuilder criteriaBuilder, QueryFilter input)
    {
        ...
        else if (fieldClass.isAssignableFrom(Instant.class))
        {
            var tuple = getInstantBindInfo(root, input);
            return criteriaBuilder.equal(tuple.item1, tuple.item2);
        }
        else
        {
